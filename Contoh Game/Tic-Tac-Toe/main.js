	...
	$("#board tr td").click(function () {
		if ($(this).text() == "" && play) {
			...
			if (checkForWinner() != -1 && checkForWinner() != "") {
				if (checkForWinner() == "X") {
					$('.column').append('<div class="winner"><span>Winner</span>X</div><button onclick="location.reload();" id="reload">Play Again</button>');
					$('.winner').css('background-color', '#61892f');
					$('#reload').css('color', '#61892f');
				} else {
					$('.column').append('<div class="winner"><span>Winner</span>O</div><button onclick="location.reload();" id="reload">Play Again</button>');
			...
			//Draw
			if (move == 10 && checkForWinner() == -1 && checkForWinner() != "") {
				$('.column').append('<div class="winner"><span>No Winner</span>Draw</div><button onclick="location.reload();" id="reload">Play Again</button>');
				$('.winner').css('background-color', '#808080');
				$('#reload').css('color', '#808080');
				play = false;
			}
		}
	});
